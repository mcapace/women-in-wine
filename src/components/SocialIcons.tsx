...
export function SocialIcons({ links, variant = "light", className = "" }: SocialIconsProps) {
  const isDark = variant === "dark";
  return (
    <div className={`flex items-center gap-4 ${className}`} role="list" aria-label="Social media">
      {links.map(({ type, href, label }) => (
        <a
          ...
          className={`p-1 rounded-sm transition-colors duration-300 ${
            isDark ? "text-cream/80 hover:text-cream" : "text-slate hover:text-burgundy"
          }`}
          aria-label={label}
        >
          {icons[type]}
